...
class NeuralNetwork:
    ...
    def learn(self, input_data):
        ...
        # 中間層　→　入力層
        # 中間層にはニューロンが２つなので、δim の値も2つとなる
        # 今回はそれをリストの形で格納
        # δmoに前の層の更新前の重みをかけて中間層の出力の微分をかけている
        # ここでもシグモイド関数の微分の形となる
        delta_w_im = [
        ...
        # 重みの更新を行う
        # 中間層にはニューロンが２つ、入力層にはバイアスとニューロンが合わせて３つあるので、
        # 全部で６つの重みを更新することになる
        # それぞれ入力層の値とδimとkをかけて修正量とし、重みに足し合わせている。
...
for t in range(0, 1000):
    for data in training_data:
        neural_network.learn(data)  # 全部で100このデータがあるから、ここは100回ループされる
print(neural_network.w_im)
print(neural_network.w_mo)

# 実行
# 二重リスト　緯度と、経度が入っている。訓練のニューラルネットワークに投入して結果を見てみる。
data_to_commit = [[34.6, 138.0], [34.6, 138.18], [35.4, 138.0], [34.98, 138.1], [35.0, 138.25], [35.4, 137.6],
                  [34.98, 137.52], [34.5, 138.5], [35.4, 138.1]]
